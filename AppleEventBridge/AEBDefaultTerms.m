...
-(NSArray *)types {
    ...
    if (!data) data = @[TYPE_NAME(@"anything", '****'),
...
-(NSArray *)enumerators {
    static NSArray *data = nil;
    if (!data) data = @[ENUM_NAME(@"yes", 'yes '),
                        ENUM_NAME(@"no", 'no  '),
                        ENUM_NAME(@"ask", 'ask '),
...
-(NSArray *)properties {
    static NSArray *data = nil;
    if (!data) data = @[PROP_NAME(@"class", 'pcls'),
                        PROP_NAME(@"id", 'ID  ')];
    return data;
}
...
-(NSArray *)elements {
    static NSArray *data = nil;
    if (!data) data = [[NSArray alloc] init];
    return data;
}

-(NSArray *)commands {
    static NSArray *data = nil;
    if (!data) data = @[COMMAND_NAME(@"run", 'aevt', 'oapp'),
